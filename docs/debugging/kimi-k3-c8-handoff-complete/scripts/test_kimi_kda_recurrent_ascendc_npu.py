...
@torch.inference_mode()
def test_kimi_k3_recurrent_kda_aclgraph_capacity32_active17():
    """Reproduce the production batch-32 graph boundary with 17 active rows."""
    from torch import nn

    from vllm_ascend.utils import enable_custom_op
    from vllm_ascend.ops.kimi_kda import AscendKimiGatedDeltaNetAttention

    # Standalone pytest does not go through NPUPlatform.import_kernels().
    # Load the C++ binding and its vendor op library before resolving
    # torch.ops._C_ascend.recurrent_kda.
    assert enable_custom_op(), "vllm-ascend custom-op extension did not load"

    torch.manual_seed(20260808)
    device = torch.device("npu")
    tokens, active_tokens, heads, dim = 32, 17, 6, 128
    state_capacity = 40

    q = torch.zeros(1, tokens, heads, dim, dtype=torch.bfloat16, device=device)
    k = torch.zeros_like(q)
    v = torch.zeros_like(q)
    raw_gate = torch.zeros(1, tokens, heads, dim, dtype=torch.float32, device=device)
    beta = torch.zeros(1, tokens, heads, dtype=torch.float32, device=device)
    q[:, :active_tokens].normal_()
    k[:, :active_tokens].normal_()
    v[:, :active_tokens].normal_()
    raw_gate[:, :active_tokens].normal_(std=0.25)
    beta[:, :active_tokens].uniform_()

    cu_values = list(range(active_tokens + 1)) + [active_tokens] * (tokens - active_tokens)
    cu_seqlens = torch.tensor(cu_values, dtype=torch.int32, device=device)
    state_values = list(range(active_tokens)) + [0] * (tokens - active_tokens)
    state_indices = torch.tensor(state_values, dtype=torch.int64, device=device)
    state_initial = torch.randn(
        state_capacity, heads, dim, dim, dtype=torch.float32, device=device
    ) * 0.01
    state_graph = state_initial.clone()
    ...
    layer.A_log = nn.Parameter(
        torch.randn(1, 1, heads, 1, dtype=torch.float32, device=device) * 0.05
    )
    layer.dt_bias = nn.Parameter(
        torch.randn(heads, dim, dtype=torch.float32, device=device) * 0.05
    )
    ...
    q[:, :active_tokens].copy_(torch.randn_like(q[:, :active_tokens]))
    k[:, :active_tokens].copy_(torch.randn_like(k[:, :active_tokens]))
    v[:, :active_tokens].copy_(torch.randn_like(v[:, :active_tokens]))
    raw_gate[:, :active_tokens].copy_(
        torch.randn_like(raw_gate[:, :active_tokens]) * 0.25
    )
    beta[:, :active_tokens].copy_(torch.rand_like(beta[:, :active_tokens]))
    state_graph.copy_(state_initial)
    graph.replay()
    torch.npu.synchronize()
    ...
    # Comparing graph replay only with eager is insufficient here: both paths
    # launch the same capacity-32 kernel and can reproduce the same per-core
    # corruption.  The CPU recurrence is independent of the AscendC task
    # scheduler, so it identifies whether the kernel itself is already wrong
    # before graph replay is considered.
    a_log_cpu = layer.A_log.detach().cpu().reshape(-1)
    dt_bias_cpu = layer.dt_bias.detach().cpu()
    state_initial_cpu = state_initial.cpu()
    ref_out, ref_state = recurrent_kda_reference(
        q.cpu(),
        k.cpu(),
        v.cpu(),
        ...
        ssm_state_indices=torch.tensor(state_values, dtype=torch.int64),
        A_log=a_log_cpu,
        dt_bias=dt_bias_cpu,
        layout="BSND",
        scale=dim**-0.5,
    ...
    torch.testing.assert_close(
        eager_out[:, :active_tokens].cpu(),
        ref_out[:, :active_tokens],
        rtol=0.02,
        atol=0.02,
    )
    torch.testing.assert_close(state_eager.cpu(), ref_state, rtol=0.02, atol=0.02)
    torch.testing.assert_close(
        graph_out[:, :active_tokens].cpu(),
        ref_out[:, :active_tokens],
        rtol=0.02,
        atol=0.02,
    )
    torch.testing.assert_close(state_graph.cpu(), ref_state, rtol=0.02, atol=0.02)
